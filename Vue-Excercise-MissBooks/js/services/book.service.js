...
function getBooksFromGoogle(filter) {
    var filteredBook = {}
    var books = []
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${filter}`)
        .then(res => {
            filteredBook = res.data.items.map(book => {
                return filteredBook = {
                    book
                }
            });
...
function createBook(book) {
    ...
    var bookDets = book.volumeInfo
    var newBook = {

        id: book.id,
        title: bookDets.title,
        subtitle: (!bookDets.subtitle) ? 'No subtitle given' : bookDets.subtitle,
        authors: (!bookDets.authors) ? 'No authors given' : bookDets.authors,
        publishedDate: (!bookDets.publishedDate) ? 'No publish date given' : bookDets.publishedDate,
        description: (!bookDets.description) ? 'No description given' : bookDets.description,
        pageCount: (!bookDets.pageCount) ? 'No page count given' : bookDets.pageCount,
        categories: (!bookDets.categoreis) ? 'No categoreis given' : bookDets.categoreis,
        thumbnail: (!bookDets.imageLinks) ? 'https://image.shutterstock.com/image-vector/no-image-available-vector-illustration-260nw-744886198.jpg' : bookDets.imageLinks.thumbnail,
        language: (!bookDets.language) ? 'No language given' : bookDets.language,
        listPrice: {
            amount: (!bookDets.listPrice) ? 'Item is not for sale' : book.saleInfo.listPrice.amount,
            currencyCode: (!bookDets.listPrice) ? '' : book.saleInfo.listPrice.currencyCode,
            isOnSale: false
